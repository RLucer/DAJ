...
$consulta = "SELECT iddb_persona,apaterno,amaterno,nombres,apodo,fnacimiento,comuna,direccion,estado,ultimacomuna
    from db_personas
    join db_comuna
    on db_personas.idcomuna=db_comuna.iddb_comuna
    join db_estado
    on db_personas.idestado=db_estado.iddb_estado
    where ultimacomuna='$comu'
...
$pdf->Cell(80, 6, 'Delincuentes por comunas donde se vio por última vez ', 0, 1, '');
...
foreach ($resultado as $fila) {
    ...
    $pdf->Cell(20, 6, utf8_decode($fila["fnacimiento"]), 1, 0, 'C');
    $pdf->Cell(20, 6, utf8_decode($fila["apodo"]), 1, 0, 'C');
    $pdf->Cell(30, 6, utf8_decode($fila["estado"]), 1, 1, 'C');
   // $pdf->Cell(20, 6, utf8_decode($fila["direccion"]), 1, 1, 'C');
